...
1. Write → run → fix until green — never done without passing run
2. Cover: happy path + 2 edge cases + 1 error case
3. Mock externals (Anthropic, Supabase, Stripe) — no real API calls
4. e2e: critical flows only · screenshot on failure
5. No snapshot tests · no sleep without justification
